...
4. 若 flow 是 `bugfix`，額外顯示：
   - 讀取 `fix_attempts`；若 `fix_attempts >= 3`，顯示：
     > ⚠️ 修復已嘗試 {fix_attempts} 次仍未解決。建議評估是否改用 `/flow-maintain`，將此問題重新拆解為多節點戰略任務（可能是設計層面問題，非單一 bug）。

5. **不要直接執行下一步**，只給建議。使用者要主動回「ok 繼續」或直接下指令，才實際進入下個階段。
